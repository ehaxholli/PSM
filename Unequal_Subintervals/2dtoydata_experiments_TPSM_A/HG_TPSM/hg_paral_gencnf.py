...
parser.add_argument('--results_dir', type=str, default="./results_paralcnf")
...
for step_no in range(1,args.nblocks+1):
    ...
    if __name__ == '__main__':
        ...
        with torch.no_grad():
            ...
            if step_no==1:
                times=np.linspace(1, 0.1+0.0001, viz_timesteps)
            else:
                times=np.linspace(0.1+0.0001,0.0001, viz_timesteps)
            ...
            if step_no==1:
                times=np.linspace(0.0001, 0.1+0.0001, viz_timesteps)
            else:
                times=np.linspace(0.1+0.0001,1+0.0001, viz_timesteps)   
