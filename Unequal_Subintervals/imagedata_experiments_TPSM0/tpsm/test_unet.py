...
if args.gen_sde:
    ...
    for model_no in reversed(range(1,args.nblocks+1)):    
        ...
        if model_no==1:
            int_interv=np.linspace(0.1, 0, gensde_steps//10)
        elif model_no==2:
            int_interv=np.linspace(1, 0.1, 9*(gensde_steps//10))
        ...
        with torch.no_grad():
            for t in torch.tensor(int_interv).to(device):
...
if args.gen:
    ...
    for model_no in reversed(range(1,args.nblocks+1)):
        ...
        if model_no==1:
            int_interv=np.linspace(0.1, 0, args.gen_steps//10)
        if model_no==2:
            int_interv=np.linspace(1, 0.1, 9*(args.gen_steps//10))
        ...
        with torch.no_grad():
            ...
            batch = odeint(
                func_wrap,
                batch,
                torch.tensor(int_interv).to(device),
                atol=1e-5,
                rtol=1e-5,
                method='rk4',
            )
...
if args.lkh:  
    ...
    for _, (testdatabatch, y) in enumerate(test_loader):
        ...
        for model_no in range(1,args.nblocks+1):
            ...
            if model_no==1:
                int_interv=np.linspace(0.001, 0.1, lkh_steps//10)
            if model_no==2:
                int_interv=np.linspace(0.1, 1, 9*(lkh_steps//10))
